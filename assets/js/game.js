...
var fight = function(enemy) {
    ...
    // randomly changes attack turn
    if (Math.random() > .5) {
        isPlayerTurn = false;
    }
    
    while (playerInfo.health > 0 && enemy.health > 0) {
        // player gets to attack first due to it being "true" still
        if (isPlayerTurn) { 
            ...
            // Fight: generate random damage value based on player's attack power
    ...
    for (var i = 0; i < enemyInfo.length; i++) {
        ...
        if (playerInfo.health > 0) {
            ...
            if (playerInfo.health > 0 && i < enemyInfo.length - 1) {
                // ask player if they want to use the shop before next round
...
var endGame = function() {
    ...
    // checking high score, if none, then 0
...
var shop = function() {
  ...
  var shopOptionPrompt = window.prompt(
    ...
    // converts reseponse to integer - whatever that means
